...
public class IOUtils extends Object {
	...
	*/
	...
	/** Testing code only - not part of the API. */
	static public void main(String[] args) throws Exception {
		switch(2) {
			case 0:
                ...
                System.out.println("\n The Inputed Hashtable = " + theDict);
                System.out.println("\n The Outputed encoding = " + IOUtils.urlEncodeMap(theDict) + "\n");
				break;

			case 1:
				InputStream input = new ByteArrayInputStream("Now is the time for \nall good men to \ncome to the aid of their \ncountry.".getBytes());
				byte[] sequence = "aid".getBytes();
				OutputStream output = new ByteArrayOutputStream();
				long n = writeInputToOutput( input, output, sequence);
				System.out.println( "\n\nTest passed (" + n + "): " + output.toString() );
				break;

			case 2:
				URL passwordUrl = new URL("http://jello:jello@bhg_xserve_ams.360works.com/fmi/xml/FMPXMLRESULT.xml?-db=Cities&-lay=xml&-findall");
				String result = urlConnectionAsString( passwordUrl.openConnection() );
		...
		//setObjectForKey(Object object, Object key)
		// Testing urlEncodeMap() method
                /*
		NSMutableDictionary theDict = new NSMutableDictionary();
		theDict.setObjectForKey("Jesse Barnum", "name");
		theDict.setObjectForKey("30022", "zip code");
		NSArray petArray = new NSArray( new String[] {"Megan", "Buddha"} );
		theDict.setObjectForKey(petArray, "pets");

		System.out.println( IOUtils.urlEncodeMap(theDict) );
                */

                // create dictionary

		//Sample of how to utilize the URLConnection and InputStream methods
		//URL theUrl = new URL("https://secure.bbqsource.com:8443/FMPro");
		//URLConnection theConnection = theUrl.openConnection();
		//postDataToUrlConnection("-format=-dso_xml&-dbnames", theConnection);
		//System.out.println( urlConnectionAsString(theConnection) );

		//System.out.println( getUrlContents("https://secure.southlandloghomes.com") );

		//Example of how to use the file methods
		/*File inputFile = new File("/Users/jesse/Downloads/ExchangeReport.xls");
		System.out.println("Contents of " + inputFile + ": " + fileAsString(inputFile));*/

		//File testUniqueFile = new File("/Users/jesse/headers");
		//System.out.println(ensureUniqueFilename( testUniqueFile ));
	}
